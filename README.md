...
* [`src/clj/full_stack_clj/`](src/clj/full_stack_clj/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/cljs/full_stack_clj/`](src/cljs/full_stack_clj/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/full_stack_clj/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `full-stack-clj` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/full_stack_clj/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns full-stack-clj.example
  (:require [full-stack-clj.config :as config])
...
```sh
port=2000 java -jar target/full-stack-clj.jar
```
